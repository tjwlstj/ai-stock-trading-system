...
- **버전**: v1.3.3-beta
- **출시일**: 2026-01-19
- **주요 변경사항**: 프론트엔드 빌드 버그 수정, 보안 취약점 3개 해결
- **상태**: 테스트 중
...
### v1.3.3-beta (2026-01-19)
**유형**: 정기 점검, 버그 수정 및 보안 패치

**변경사항**:
- 프론트엔드 빌드 버그 수정
  - 누락된 `src/lib/utils.js` 파일 생성
  - `cn()` 유틸리티 함수 추가 (clsx + tailwind-merge)
- 프론트엔드 보안 취약점 3개 해결
  - `hono` < 4.11.4 (JWT algorithm confusion, 2개 취약점)
  - `tar` <= 7.5.2 (Arbitrary file overwrite)
  - `pnpm audit --fix`로 자동 패치 적용

**패키지 현황**:
- 백엔드 패키지: 변경 없음 (모두 최신 버전)
- 프론트엔드 패키지: 보안 overrides 추가
- 보안 취약점: 2개 (간접 의존성, 실질적 위험 낮음)

**파일 변경**:
- `frontend/src/lib/utils.js`: 누락된 유틸리티 파일 생성
- `frontend/package.json`: 보안 overrides 추가 (hono, tar)
- `frontend/pnpm-lock.yaml`: 보안 패치 적용
- `VERSION`: 1.3.3-beta
- `CHANGELOG.md`: v1.3.3-beta 항목 추가
- `VERSION_HISTORY.md`: 베타 버전 히스토리 업데이트

**테스트 결과**:
- ✅ 프론트엔드 빌드 성공 (dist 생성 완료)
- ✅ `pydantic.v1` 미사용 확인 (FastAPI 0.128.0 호환 가능)
- ⚠️ 프론트엔드 보안 감사: 2개 취약점 남음 (간접 의존성)
...
**참고 문서**:
- [점검 발견사항](./inspection_findings_2026-01-19.md)
- [점검 분석 보고서](./inspection_analysis_2026-01-19.md)

---

### v1.3.2-beta (2026-01-12)
**유형**: 정기 점검, 보수적 업데이트 및 보안 패치

**변경사항**:
- `aiosqlite` 0.21.0 → 0.22.1 업데이트
  - 버그 수정 및 성능 개선
- 프론트엔드 보안 취약점 7개 해결
  - `qs`, `@modelcontextprotocol/sdk`, `react-router` 등
  - `pnpm audit --fix`로 자동 패치 적용

**패키지 현황**:
- 전체 패키지: 15개
- 업데이트된 패키지: 1개 (aiosqlite)
- 최신 버전 유지: 14개
...
- `VERSION`: 1.3.2-beta
- `CHANGELOG.md`: v1.3.2-beta 항목 추가
- `VERSION_HISTORY.md`: 베타 버전 히스토리 업데이트

**테스트 결과**:
- ✅ aiosqlite 0.22.1 import 및 기능 테스트 통과
- ✅ 프론트엔드 보안 감사 통과 (취약점 0개)
...
**참고 문서**:
- [패키지 점검 보고서](./package_check_2026-01-12.md)
- [FastAPI 릴리스 분석](./fastapi_releases_2026-01-12.md)
